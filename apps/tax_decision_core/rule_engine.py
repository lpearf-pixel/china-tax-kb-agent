...
def _coerce_pair(left: Any, right: Any) -> tuple[Any, Any]:
    if isinstance(right, (int, float, Decimal)) and isinstance(
        left, (str, int, float, Decimal)
    ):
        try:
            return Decimal(str(left).strip()), Decimal(str(right))
        except (InvalidOperation, ValueError):
            pass
    if isinstance(left, Decimal) and isinstance(right, str):
        try:
            return left, Decimal(right.strip())
        except (InvalidOperation, ValueError):
            pass
...
class RuleEngine:
    def _condition(
        ...
        facts: FactGraph | Mapping[str, Any],
        ...
        if "all" in node:
            ...
            for child in node["all"]:
                ...
                if result is False:
                    return False, sorted(set(missing)), trace
                if result is None:
                    saw_unknown = True
        ...
        if "any" in node:
            ...
            for child in node["any"]:
                ...
                trace.extend(child_trace)
                if result is True:
                    return True, sorted(set(missing)), trace
                if result is None:
                    saw_unknown = True
    ...
    def evaluate(
        ...
        if not rule.is_effective_on(valid_on):
            return RuleEvaluation(
                ...
                trace=[
                    {
                        "reason": "rule_not_effective_on_date",
                        "valid_on": valid_on.isoformat(),
                    }
                ],
                legal_basis=[item.to_mapping() for item in rule.legal_basis],
                priority=rule.priority,
            )
        missing_required = sorted(
            {
                fact_id
                for fact_id in rule.requires
                if _value_from_facts(facts, fact_id) is MISSING
            }
        )
        if missing_required:
            return RuleEvaluation(
                rule_id=rule.rule_id,
                status=RuleEvaluationStatus.INSUFFICIENT_FACTS,
                missing_fact_ids=missing_required,
                trace=[
                    {"reason": "required_facts_missing", "facts": missing_required}
                ],
                legal_basis=[item.to_mapping() for item in rule.legal_basis],
                priority=rule.priority,
            )
        ...
        manual = bool(rule.then.get("manual_review_required")) or (
            rule.then.get("confidence") == "manual_review"
        )
        return RuleEvaluation(
            rule_id=rule.rule_id,
            status=(
                RuleEvaluationStatus.MANUAL_REVIEW_REQUIRED
                if manual
                else RuleEvaluationStatus.APPLICABLE
            ),
            outcome=str(rule.then.get("outcome") or ""),
            value=rule.then.get("value"),
            trace=trace,
            legal_basis=[item.to_mapping() for item in rule.legal_basis],
            priority=rule.priority,
        )
    ...
    def evaluate_all(
        ...
        applicable_statuses = {
            RuleEvaluationStatus.APPLICABLE,
            RuleEvaluationStatus.MANUAL_REVIEW_REQUIRED,
        }
        ...
        for group, candidates in by_group.items():
            ...
            signatures = {
                (evaluation.outcome, repr(evaluation.value)) for _, evaluation in top
            }
            if len(signatures) > 1:
                conflicts.append(
                    RuleEvaluation(
                        rule_id=f"conflict:{group}",
                        status=RuleEvaluationStatus.CONFLICT,
                        outcome=group,
                        trace=[
                            {
                                ...
                                "rules": [rule.rule_id for rule, _ in top],
                            }
                        ],
                        priority=top_priority,
                    )
